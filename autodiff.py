...
class sigmoid(expression):
    ...
    def diff(self, values, diffto):
        # chain rule
        # Use numerically stable form: sigmoid(z) * (1 - sigmoid(z)) * z'
        s = self.eval(values)
        return s * (1 - s) * self.first.diff(values, diffto)
...
def basictests():
    ...
    print('diff of 2 ** x, d/dx is 709.782712893: ', round(power2.diff(vals, 'x'), 3) == 709.783)
